...
public class DenyGroupMemberCommandHandler(
    IMessageGroupRepository groupRepo,
    ICurrentUserService currentUser,
    ISender sender)
    : IRequestHandler<DenyGroupMemberCommand, Result>
{
    public async Task<Result> Handle(DenyGroupMemberCommand command, CancellationToken ct)
    {
        ...
        if (group.Type == MessageGroupType.Direct)
            return Result.Failure(Error.Validation(
                ...
                "Không thể từ chối thành viên cho chat 1-1."));
        ...
        if (callerMember is null || callerMember.Role is not (GroupMemberRole.Owner or GroupMemberRole.Manager))
            return Result.Failure(Error.Forbidden(
                ...
                "Chỉ owner hoặc manager mới được từ chối thành viên."));
        ...
        var sendResult = await sender.Send(new SendMessageCommand(
            command.GroupId,
            $"{actorName} đã từ chối {targetName} vào nhóm".Trim(),
            null,
            null,
            MessageType.MemberDenied,
